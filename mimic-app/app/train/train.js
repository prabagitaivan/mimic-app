...
const frame = dataset.datatrain[0].data.length;
const sample = dataset.datatrain[0].data[0].length;
const inputShape = [frame, sample, 1];
...
const epochs = 64; // n times the batches to be trained.
const trainIteration = 250; // n times the train iteration --> datasize / batch.
const testIteration = 10; // Every n times in `trainIteration` the `model` is tested.

// define all test input and output from `dataset.datatest` to `testData` and `testLabel` for prediction.
const testData = nextBatch.getData(dataset.datatest, i, dataset.datatest.length, frame, sample);
...
async function train() {
  for (i = 0; i < trainIteration; i++) {
    ...
    const batchData = nextBatch.getData(dataset.datatrain, i, batch, frame, sample);
    const batchLabel = nextBatch.getLabel(dataset.labels, dataset.datatrain, i, batch);
    const batchValidation = nextBatch.getData(dataset.datavalidation, i, batch, frame, sample);
